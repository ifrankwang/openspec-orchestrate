...
description: Java 后端数据访问与外部集成安全规范——MyBatisPlus、POI、LangChain4j、文件上传、凭证管理。适用场景：安全审查（code-reviewer-security）、性能审查（code-reviewer-performance）、编写代码（backend-developer）。
---
...
```java
// 正确：参数绑定
@Select("SELECT * FROM lr_loan_info WHERE id = #{id}")
...
**PO ↔ Domain 转换**：使用 MapStruct Converter（同 java-ddd-architecture 中的 MapStruct 规范）
