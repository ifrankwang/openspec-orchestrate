...
| 6.5 | SCA 依赖漏洞扫描 | 第 6.5 节 | 报告 Critical/High/Medium 级别漏洞数及清单 |
| 8 | 质量工具配置检查 | 第 7 节 | 报告通过或配置削弱清单 |
| 9 | 汇总与提交 | 第 8 节 | 提交检查结果 |
...
| OWASP Dependency-Check | `mvn dependency-check:help -q` 或 `docker images owasp/dependency-check` | 可自愈 | 先 `docker pull owasp/dependency-check` 恢复；失败则生成 issue（dimension=maintainability）并提请用户 |
...
安装 OWASP Dependency-Check 或等位工具：
- Maven 插件：在 `pom.xml` 中配置 `dependency-check-maven-plugin`
...
工具不可用时按 quality-gate §0 处理。若为工具缺失，生成 High 级别 issue（dimension=maintainability），记录缺失工具名称与安装指引。安装指引：Maven 配 `dependency-check-maven-plugin`，或 `docker pull owasp/dependency-check`。

### 违规项 → issue 映射

| Dependency-Check severity | issue severity |
|--------------------------|---------------|
| CRITICAL | Critical |
| HIGH | High |
| MEDIUM | Medium |
