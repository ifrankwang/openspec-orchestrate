...
`opx_reviewer_submit` 不接受 `dimension` 参数，维度通过 `DIMENSION_AGENT_MAP` 从 `context.agent` 反查。不允许调用者自行传入维度。

### 豁免按"谁提谁裁定"原则

提出 issue 的角色负责裁定该 issue 的豁免申请。Phase 2 由 validator 裁定，Phase 3 由对应维度 reviewer 裁定。架构师仅以 architecture reviewer 身份通过 `opx_reviewer_submit(exempt_issue_ids=[...])` 裁定自己报的 issue。
