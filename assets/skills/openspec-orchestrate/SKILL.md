...
- **开发**（`openspec-developer`）：Phase 2 dev_impl——实施 task / Phase 3 Issue 修复
- **Review 三层门禁**：
...
编排者分派 `openspec-reviewer-task`。task reviewer 验证 task 产出完整性、启动服务、健康检查、测试接口、审查测试代码质量。
...
- **存在不通过维度（retry ≤ 3）**：分派 `openspec-developer`（fixer 模式）修复，若有豁免申请先分派对应维度 quality reviewer 裁定。
