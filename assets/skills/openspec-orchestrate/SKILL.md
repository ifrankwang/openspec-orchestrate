...
- **架构师**（`openspec-architect`）：Phase 1 文档一致性复核；Phase 3 收尾豁免裁定
...
工具自行解析 `tasks.md`。`recovery` 参数按 phase 恢复阶段状态（< phase 为 `completed`，== phase 为 `in_progress`）。同 changeId 可重复调用，仅重建当前组。部分启动过的组须编排者通过 `opx_orch_init(recovery=...)` 显式恢复。
...
**豁免申请**：developer 对无法修复的 issue 可申请豁免，由 Phase 3 的对应维度 reviewer 裁定。
...
4. **调用 `opx_orch_complete_task_group(merge_target="<target>")`**：
   - 工具校验 worktree clean、无残留 blocking issue、无未完成 task
   - 工具执行 `git checkout <merge_target>` + `git merge --no-ff <task-group-branch>` 合并分支
   - 合并冲突时工具中止合并并返回 blocked，保留 worktree/分支；编排者向用户报告，手动解决冲突后重试
   - 合并成功 → 工具执行 `git worktree remove` + `git branch -D` 清理
    - 任务组 status=completed，currentTaskGroupId 推进到下个 pending（不自动 begin——编排者需显式调 init）
