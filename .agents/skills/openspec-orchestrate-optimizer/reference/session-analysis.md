...
从合规分析结果和 session 行为模式中，识别需要改进的具体对象。**每条建议必须指名具体文件路径和章节/行号**（如 `openspec-reviewer-architecture.md` 的第 X 行、`assets/skills/*/SKILL.md` 的某个章节等）。
