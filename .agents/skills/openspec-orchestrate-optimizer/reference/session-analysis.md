...
执行 `scripts/export-session.sh <sessionID>`，返回精简 JSON 文件路径。脚本同时生成同名的 `.summary.jsonl` 摘要文件（每消息一行的结构化概览：消息索引、sessionID、role、agent、tool 列表、文本摘要），供后续 Step 3 制定分段策略时快速查询。

如果 sessionID 未提供，向用户询问。如果脚本报错（如 sessionID 不存在、jq 或 opencode 不可用），将错误信息展示给用户并确认后重试。

脚本内置的递归深度上限为 5 层，主 session 和所有子 session 的 messages 合并到同一个文件中。精简规则由脚本内置，主流程无需关心。

### 2. 理解编排规范并制定分析计划

读取以下两份文档，一次性完成以下三项输出：

- `openspec-orchestrate` SKILL.md（`.opencode/skills/openspec-orchestrate/SKILL.md`）：编排工作流的阶段划分、各 Phase 约束、自检清单
- `openspec-orchestrator` agent 定义（`.opencode/agents/openspec-orchestrator.md`）：编排者自身的禁止操作、子代理分派规则、权限约束
...
改进对象的覆盖范围包括但不限于：
- agents（`*.opencode/agents/*.md` 中的哪个 agent 定义，哪部分指令）
- skills（`*.agents/skills/*/SKILL.md` 中的哪个 skill，哪个章节）
- tools（`*.opencode/tools/*.ts` 中的哪个工具，哪个校验/状态逻辑）
...
在向用户展示最终报告前，必须完成以下自检，逐条确认：

- [ ] 重读本轮全部用户反馈（包括用户主动要求、确认、或拒绝的每个点），逐条确认已在报告中处理，无遗漏
- [ ] 每条改进建议是否指名了具体文件路径和行号（或章节名），无模糊引用
- [ ] 每条根因是否已交叉验证——指向文件的已读文件对应章节确认，指向工具的已根据工具行为确认逻辑
- [ ] 所有改进建议是否均基于目标文件的实际内容，而非推测；是否均已先读文件再提建议
- [ ] 不合规项清单是否与 Step 2 的约束清单逐一核对，确认无漏检
...
- 主代理不直接读取 session JSON，必须通过子代理分段读取
