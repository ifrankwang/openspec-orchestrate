...
- **输出前自检**：逐条确认——用户反馈无遗漏、建议指向明确且基于文件实际内容、根因/现状经交叉验证、不违反 AGENTS.md 治理原则
- **输出报告**：纯段落格式，不落盘
...
输出分析报告后，如需对分析结果实施修复，主代理按 `reference/remediation.md` 编排修复流水线（含用户确认的修复项清单 + 来源分析报告作为交接载体）。**主代理禁止直接 edit/write 任何文件**。开发+测试、复核、文档+版本分别由独立子代理串行执行；执行模式与收尾规则以 `reference/remediation.md` 为准。
