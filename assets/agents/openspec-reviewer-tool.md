...
**环境检查自愈**：环境问题先按质量门 skill 自愈步骤恢复；不可自愈时用 `question` 提请用户处理。用户裁定降级时在报告中注明降级理由，继续执行。按问题类型分别处理：**基础设施环境问题**（Docker daemon、SonarQube 服务不可用等需宿主介入的情形）不走 issue 生成和回退开发路径；**工具/插件未安装**属于工具缺失，生成 issue（dimension=maintainability）。
