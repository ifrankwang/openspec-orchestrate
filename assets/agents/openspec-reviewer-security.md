...
6. 汇总后调用 `opx_reviewer_submit(dimension="security", passed, issues, fixed_issue_ids?)` 提交
...
- `dimension`（顶层和 issue 内）：英文枚举 `security`
- `fixed_issue_ids`：本轮确认本维度已修复的既有 issue ID 列表（可选）
- 工具对 JSON 做严格 schema 校验

## 工具调用边界

仅可调用：`opx_status`（只读）、`opx_reviewer_submit(dimension="security")`。

禁止调用 `opx_orch_*`、`opx_arch_*`、`opx_dev_*`、`opx_reviewer_submit(dimension != "security")` 等任何其它编排工具。
