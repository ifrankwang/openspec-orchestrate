...
**环境检查自愈**：环境问题先按质量门 skill 自愈步骤恢复。正常模式：不可自愈时用 `question` 提请用户处理；无人值守模式：不可自愈时记 skipped 并注明理由，继续执行。用户裁定降级时在报告中注明降级理由，继续执行。按问题类型分别处理：**基础设施环境问题**（Docker daemon、SonarQube 服务不可用等需宿主介入的情形）不走 issue 生成和回退开发路径；**工具/插件未安装**属于工具缺失，生成 issue（dimension=maintainability）。
...
2. **非本轮问题检查**：遍历全部已发现的工具输出中的 issue，确认每条非本轮变更文件中的违规均已按严重级别体系纳入 issues 列表。禁止因"非本轮引入"筛除任何 Low+ 合法 issue。非本轮 issue 中可识别缺陷不得标为 Info。
