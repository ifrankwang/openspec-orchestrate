...
| 2 | 路径统一 `{context-path}/api/v1/` 前缀 + kebab-case | 如 `{context-path}/api/v1/orders/{orderId}/items`；前缀通过 `server.servlet.context-path={context-path}` + Controller 映射 `/api/v1/...` 组合实现 |
